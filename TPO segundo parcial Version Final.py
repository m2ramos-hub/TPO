...
def buscar_indice_por_nombre(inventario, nombre_buscado):
    # recorre el inventario comparando en minusculas para evitar problemas
    # de mayusculas. Retorna el indice si lo encuentra, -1 si no existe.
...
def modificar_precio(inventario):
    ...
    mostrar_inventario(inventario)
    if len(inventario) > 0:
        id_str = input("ID del producto (salir para cancelar): ").strip()
        if id_str == "salir":
            print("  Operacion cancelada.")
        else:
            indice = buscar_indice_por_id(inventario, id_str)
...
def main():
    ...
    mostrar_menu()
    opcion = input("  Opcion: ").strip() #.strip() elimina los espacios en blanco al principio y al final del texto ingresado.
